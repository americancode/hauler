...
func TestResolveChartJobs_ChartPrefixDoesNotPrefixImages(t *testing.T) {
	annotations := map[string]string{consts.ChartAnnotationPrefix: "helm-charts/"}

	jobs, err := resolveChartJobs(&flags.SyncOpts{}, annotations, "/manifests", []v1.Chart{{Name: "my-chart-name"}})
	if err != nil {
		t.Fatalf("resolveChartJobs: %v", err)
	}

	if jobs[0].rewrite != "helm-charts/my-chart-name" {
		t.Errorf("chart rewrite = %q, want helm-charts/my-chart-name", jobs[0].rewrite)
	}
	if jobs[0].imagePrefix != "" {
		t.Errorf("imagePrefix = %q, want empty when image-prefix is absent", jobs[0].imagePrefix)
	}
}
